trigger updateEvtSubjectOnOpp  on Event (after insert, after update, after delete) {
     ...
     Time zeroTime = Time.newInstance(0,0,0,0);
            ...
            if(t.WhatId != null && String.valueOf(t.whatId).startsWith('006')){
               /* if(Trigger.isInsert){
                    ...
                    if( t.Subject != Trigger.oldMap.get(t.Id).subject ||
                        t.startDateTime != trigger.oldMap.get(t.Id).startDateTime){
                        activityOppIds.add(t.whatid);
                    }
                }*/
                activityOppIds.add(t.whatid);
      ...
      if(!activityOppIds.isEmpty()){
          for(Opportunity opp : [SELECT ID,Open_Activities__c, (Select Id, Subject,ActivityDate,Owner.name,who.name From Tasks where isClosed = false and RecurrenceActivityId = null order by ActivityDate desc),(Select Subject,startdatetime,owner.name,who.name from Events where RecurrenceActivityId=null and StartDateTime >= TODAY order by startdatetime desc)  FROM Opportunity WHERE ID IN:activityOppIds
